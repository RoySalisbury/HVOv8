...
{
    public sealed partial class VoltronicInverter : IDisposable
    {
        ...
        private readonly ILogger<VoltronicInverter> _logger;
        private readonly VoltronicInverterOptions _options;

        private readonly InverterSerialCommunicationsClient _client;
        private readonly Stopwatch _lastPoll = Stopwatch.StartNew();

        public VoltronicInverter(ILogger<VoltronicInverter> logger, VoltronicInverterOptions options)
        {
            this._logger = logger;
            this._options = options;

            this._client = new InverterSerialCommunicationsClient(options);
        }
        ...
        private async Task<(bool IsSuccess, ReadOnlyMemory<byte> Data)> SendRequest(ReadOnlyMemory<byte> request, int receiveTimeout = 750, CancellationToken cancellationToken = default)
        {
            ...
            using (SemaphoreSlim semiphoreLock = new SemaphoreSlim(1))
            {
                if (await semiphoreLock.WaitAsync(5000, cancellationToken))
                {
                    ...
                    while (_lastPoll.ElapsedMilliseconds < MaxPollingRate.TotalMilliseconds)
                    {
                        ...
                        this._logger.LogWarning("Delaying for correct polling rate: {waitTime}", waitTime);
                    ...
                    await this._client.FlushAsync(cancellationToken);
                    try
                    {
                        // The underlying system is a HID device. The structure of this is designed so that specifc size pages are 
                        ...
                        do
                        {
                            ...
                            await this._client.WriteAsync(packet, cancellationToken);
                            await this._client.FlushAsync(cancellationToken);
                        ...
                        return await ReceiveResponse(receiveTimeout: receiveTimeout, cancellationToken);
                    }
                    catch (Exception)
                    {
        ...
        private async Task<(bool IsSuccess, ReadOnlyMemory<byte> Data)> ReceiveResponse(int receiveTimeout = 500, CancellationToken cancellationToken = default)
        {
            try
            {
                var packetSize = 8;
                ...
                do
                {
                    ...
                    try
                    {
                        ...
                        var b = await this._client.ReadAsync(buffer, bytesRead, packetSize, cancellationToken).WithCancellation(timeout: receiveTimeout, cancellationToken);
                        if (b == 0)
                        {
                            break;
                        }
                    ...
                    catch (TimeoutException)
                    {
                        ...
                        // This seems to clear out the issue of the stream getting locked. We should get back the original request.
                        var request = new byte[] { 0x00, 0x0D }; // <null><cr>
                        return await SendRequest(request, cancellationToken: cancellationToken);
                    }
